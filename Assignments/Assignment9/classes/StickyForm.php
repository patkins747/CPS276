...
class StickyForm extends Validation {
    ...
    public function validateForm($data, $formConfig) {
        ...
        foreach ($formConfig as $key => &$element) {
            ...
            if (isset($element['type']) && in_array($element['type'], ['password']) && isset($element['regex'])) {
                // Check if required field is empty
                if ($element['required'] && empty($element['value'])) {
                    $element['error'] = $customErrorMsg ?? 'This field is required.';
                    $formConfig['masterStatus']['error'] = true;
            ...
            if (isset($element['type']) && in_array($element['type'], ['password_confirm']) && isset($element['regex'])) {
                // Check if password confirm matches password                  
                    if($element['value'] === $data['password']){
                        $isValid = true;
                    }
                    else {
                        $element['error'] = "Passwords do not match!";
                        $formConfig['masterStatus']['error'] = true;
                    }
            }
